...
export default {
 async fetch(request, env, ctx) {
  ...
  let cache,saved;
  try { cache=await globalThis.caches?.open('aura-upstream-v1'); saved=await cache?.match(key); } catch { cache=undefined; }
  ...
  if(saved&&Date.now()-Number(saved.headers.get('x-aura-fetched-at'))<60000){body=await saved.text();source='cache';}
   ...
   try {
    ...
    if(cache)ctx.waitUntil(cache.put(key,new Response(body,{headers:{'Content-Type':types[path],'Cache-Control':'public, max-age=86400','x-aura-fetched-at':String(Date.now())}})).catch(()=>{}));
   } catch {
